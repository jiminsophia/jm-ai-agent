...
book_data_agent= Agent(
    ...
    get_book_search 독를 사용하여 현재 인기 있는 도서들을 조회하고,
...
structured_output_agnet= Agent(
    ...
    전체 추천 도서 수를 total_cout에 포함시키세요.
    """, 
    output_schema= BookList,    # 출력 스키마 지정
)

# 오케스트레이션 담당하는 루트 에이전트
root_agent= Agent(
    name= "book_recommenation_orchestrator", 
    model= "gemini-2.5-flash", 
    ...
    """, 
    sub_agents= [book_data_agent, structured_output_agnet],     # 하위 에이전트들
)