...
#텍스트를 벡터로 변환, FAISS 벡터스토어에 저장
#FAISS.from_texts()는 텍스트 리스트와 임베딩 모델을 인자로 --> 벡터들을 인메모리 인덱스에 저장
vectorstore= FAISS.from_texts(texts, embeddings)

#유사한 문서 검색
#쿼리의 벡터화는 벡터스토어 라이브러리 내에서 임베딩 알아서 해줌. k=2는 코사인 유사도가 가장 높은 2개 문서 반환하라는 것
#결괏값은 langchain.schema.Document 객체로, page_content(본문)와 metadata 포함 --> 문서를 넣고 찾을 수 있도록 의도되었음
