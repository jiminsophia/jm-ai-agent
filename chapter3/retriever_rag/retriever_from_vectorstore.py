...
# 리트리버 생성
# 벡터 스토어 -> 리트리버 변환, runnable 인터페이스 구현으로 다른 컴포넌트들과 연결 용이
# similarity는 유사도 기반으로 가장 기본이며, MMR, similarity_score_threshold 등 여러 후보가 더 있음
...
# 리트리버를 사용한 LLM 호출
# message를 통해 프롬프트에 컨텍스트를 포함시켜서, LLM이 조금 더 나은 답변을 하도록 유도
